...
//-> arrays//
let computerReihenfolge: number[] = []; // Array, in welches die Reihenfolge der aufleuchtenden Buttons des Computers gespeichert werden 
let spielerReihenfolge: number[] = []; //Array, in welches die gespielte Reihenfolge vom Spieler gespeichert wird
let sounds: string[] = ["sound_boing.mp3", "sound_laugh.mp3", "sound_squeaking.mp3" , "sound_whistle.mp3" , "sound_yeah.mp3"]; //Array, in dem alle Sounds abgespeichert sind//

//->numbers//
let buttonLeuchten: number; // Anzahl aufleuchtender buttons wird hier gespeichert
let anReihe: number; // Angabe der Spielrunde
let intervalId: number;

//->booleans//
let keineFehler: boolean; // true/false ob Nutzer Fehler macht oder keine
let computeranReihe: boolean; // true/false ob Computer an der Reihe ist
let ton: boolean = true; // soll button ton abspielen oder nicht
let on: boolean = false; // wird das Spiel gespielt? wenn true = spieler soll button klicken, wenn false = nicht klicken
let gewonnen: boolean; // true/false Spiel gewonnen oder nicht

//-> booleans für den Schwierigkeitsgrad//
//Spieler wählt einer der 4 bei Spielstart aus//
//true/false Spieler hat ___Grad ausgewählt//
let leichtchosen: boolean;
let mittelchosen: boolean;
let schwerchosen: boolean;
let sehrschwerchosen: boolean;

//step 2: Buttons selektiert//

//-> Schwierigkeitsgrad
const modeleicht: HTMLElement = document.querySelector("#gradleicht");
const modemittel: HTMLElement = document.querySelector("#gradmittel");
const modeschwer: HTMLElement = document.querySelector("#gradschwer");
const modesehrschwer: HTMLElement = document.querySelector("#gradsehrschwer");
//-> Oben/Unten/Innen//
const obenLinks: HTMLElement = document.querySelector("#obenlinks");
const obenRechts: HTMLElement = document.querySelector("#obenrechts");
const untenLinks: HTMLElement = document.querySelector("#untenlinks");
const untenRechts: HTMLElement = document.querySelector("#untenrechts");
const kreisInnen: HTMLElement = document.querySelector("#kreisinnen");
//-> Counter//
const anreiheCounter: HTMLElement = document.querySelector("#anreihe");
const befehle: Element = document.querySelector("#befehl");
...
//-> leicht//
modeleicht.addEventListener("click", function (): void { //interaktion vom Spieler, es wird auf Leicht geklickt
...
modemittel.addEventListener("click", function (): void {  //interaktion vom Spieler, es wird auf Mittel geklickt
...
modeschwer.addEventListener("click", function (): void { //interaktion vom Spieler, es wird auf Schwer geklickt
...
modesehrschwer.addEventListener("click", function (): void { //interaktion vom Spieler, es wird auf Sehr-Schwer geklickt
...
function anreiheSpiel(): void { //für den Spielablauf, wer an der Reihe ist
  ...
  if (computeranReihe) { // wenn der Computer an der Reihe ist
    ...
    setTimeout( function(): void { //
      if (computerReihenfolge[buttonLeuchten] == 1) obenlinksbutton();   //wenn die erste Stelle im Array abgefragt wird, wird der Button oben links aufgeleuchtet
      if (computerReihenfolge[buttonLeuchten] == 2) obenrechtsbutton();  //wenn die zweite Stelle im Array abgefragt wird, wird der Button oben rechts aufgeleuchtet
      if (computerReihenfolge[buttonLeuchten] == 3) untenlinksbutton();  //wenn die dritte Stelle im Array abgefragt wird, wird der Button unten links aufgeleuchtet
      if (computerReihenfolge[buttonLeuchten] == 4) untenrechtsbutton(); //wenn die vierte Stelle im Array abgefragt wird, wird der Button unten rechts aufgeleuchtet
      if (computerReihenfolge[buttonLeuchten] == 5) innenbutton();       //wenn die fünfte Stelle im Array abgefragt wird, wird der Button innen aufgeleuchtet
...
function obenlinksbutton(): void { //funktion für obenlinks
  if (ton) {
    let sound: HTMLAudioElement = new Audio(sounds[0]);  //ruft die erste Stelle (den ersten Sound) im "sounds" Array auf
...
function obenrechtsbutton(): void { //funktion für obenrechts
  if (ton) {
    let sound: HTMLAudioElement = new Audio(sounds[1]); //ruft die zweite Stelle (den zweiten Sound) im "sounds" Array auf
    sound.play();
  }
  ton = true;
...
function untenlinksbutton(): void {   //funktion für untenlinks
  if (ton) {
    let sound: HTMLAudioElement = new Audio(sounds[2]); //ruft die dritte Stelle (den dritten Sound) im "sounds" Array auf
    sound.play();
  }
  ton = true;
...
function untenrechtsbutton(): void {    //funktion für untenrechts
  if (ton) {
    let sound: HTMLAudioElement = new Audio(sounds[3]); //ruft die vierte Stelle (den vierten Sound) im "sounds" Array auf
    sound.play(); 
  }
  ton = true;
...
function innenbutton(): void {    //funktion für innen
    if (ton) {
      let sound: HTMLAudioElement = new Audio(sounds[4]);   //ruft die fünfte Stelle (den fünften Sound) im "sounds" Array auf
      sound.play();
    }
    ton = true;
...
function buttonLeuchtenColor(): void {
...
obenLinks.addEventListener("click", function (): void { //Spieler klickt auf den Button obenlinks
...
obenRechts.addEventListener("click", function (): void {    //Spieler klickt auf den Button obenrechts
  if (on) {
...
untenLinks.addEventListener("click", function(): void {    //Spieler klickt auf den Button untenlinks
  if (on) {
...
untenRechts.addEventListener("click", function (): void {    //Spieler klickt auf den Button untenrechts
  if (on) {
...
kreisInnen.addEventListener("click", function (): void {    //Spieler klickt auf den Button innen
  if (on) {
...
function pruefen(): void {   //funktion zum Überprüfen
